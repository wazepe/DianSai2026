...
/*OLED的I2C从机地址（7位地址0x3C，左移1位后为0x78）*/
#define OLED_I2C_ADDRESS    (0x3C)

/*I2C TX FIFO大小（MSPM0G3507的I2C FIFO为8字节）*/
...
/**
  ...
  * 参    数：无
  * 返 回 值：无
  * 说    明：等待I2C控制器传输完成（BUSY标志清除）
  */
static void OLED_I2C_WaitDone(void)
{
...
/**
  ...
  * 返 回 值：无
  * 说    明：通过硬件I2C发送OLED命令
  *           协议：[START][0x78][0x00][Command][STOP]
  */
void OLED_WriteCommand(uint8_t Command)
{
    uint8_t txBuf[2];

    txBuf[0] = 0x00;        // 控制字节，0x00表示写命令
    txBuf[1] = Command;     // 命令数据

    /*等待总线空闲*/
    OLED_I2C_WaitIdle();

    /*将数据填入TX FIFO（2字节：控制字节+命令）*/
    DL_I2C_fillControllerTXFIFO(I2C_OLED_INST, txBuf, 2);

    /*发起传输：目标地址 + TX方向 + 2字节数据，自动生成START和STOP*/
...
/**
  ...
  * 参    数：Count 要写入数据的数量
  * 返 回 值：无
  * 说    明：通过硬件I2C连续发送OLED显示数据
  *           协议：[START][0x78][0x40][Data0][Data1]...[DataN][STOP]
  *           由于TX FIFO大小为8字节，对于大数据量传输需要分批填充
  */
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
    uint8_t txBuf[I2C_TX_FIFO_SIZE];
    ...
    while (remaining > 0)
    {
        ...
        /*计算本次传输的字节数（控制字节1字节 + 数据最多7字节）*/
        chunkSize = remaining;
        if (chunkSize > (I2C_TX_FIFO_SIZE - 1)) {
            chunkSize = I2C_TX_FIFO_SIZE - 1;  // 留1字节给控制字节
        }

        /*构建发送缓冲区：第1字节为控制字节0x40，后续为数据*/
        txBuf[0] = 0x40;    // 控制字节，0x40表示写数据
        memcpy(&txBuf[1], &Data[dataIndex], chunkSize);

        /*将数据填入TX FIFO*/
        DL_I2C_fillControllerTXFIFO(I2C_OLED_INST, txBuf, chunkSize + 1);

        /*发起传输*/
        DL_I2C_startControllerTransfer(I2C_OLED_INST, OLED_I2C_ADDRESS,
            DL_I2C_CONTROLLER_DIRECTION_TX, chunkSize + 1);
